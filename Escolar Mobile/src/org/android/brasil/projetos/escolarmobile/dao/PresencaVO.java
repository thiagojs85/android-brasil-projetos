...
public class PresencaVO extends VOBasico {
	...
	private static final String CREATE_PRESENCA = "CREATE TABLE "
			+ TABLE_PRESENCA + "(" + COLUMN_ID
			+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_DATA
			+ " DATE NOT NULL, " + COLUMN_ID_ALUNO + " INTEGER NOT NULL, "
			+ COLUMN_FALTA + " INTEGER NOT NULL);";
	 ...
	 * @return
	 */
	public static List<PresencaVO> consultarPresenca(long idTurmaMateria) {
	...
	private static Cursor consultarPresenca(String[] key, String[] value) {
		return consultar(TABLE_PRESENCA, new String[] { COLUMN_ID, COLUMN_DATA,
				COLUMN_ID_ALUNO, COLUMN_FALTA }, key, value);
	}
	...
	public static long inserirPresenca(PresencaVO presencaoVO) {
		...
		if (consultarPresenca(presencaoVO.getIdAluno(), presencaoVO.getData())
				.isEmpty()) {
	...
	/**
	 * Remove as presenças que possuam o valor <b>value</b> na coluna
	 * <b>column</b>.
	 * 
	 * @param column
	 * @param id
	 * @return
	 */
	public static boolean removerPresenca(String column, String value) {
		Cursor cursor = consultar(TABLE_PRESENCA, new String[] { COLUMN_ID },
